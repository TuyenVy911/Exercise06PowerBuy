...
//      element = wait.until(ExpectedConditions.visibilityOfElementLocated(findBy(locator)));
    ...
    logger.info(MessageFormat.format("Timeout: ''{0}''", waitTime));
    return element;
  }
...
//    WebElement element = findElement();
  ...
  public void clickJS(String locator) {
    ...
    try {
      ...
      logger.info(MessageFormat.format("Click web element located by ''{0}'' successfully", locator));
    } catch (Exception e) {
  ...
  public Integer getNumberOfFramesByJavaScript(){
    ...
    try{
      ...
      logger.error(MessageFormat.format("Cannot get number of frame by javascript. Root cause: {0}", e.getMessage()));
    }
  ...
  public int getNumberOfFramesUsingTag(){
    ...
    try{
      ...
      logger.error(MessageFormat.format("Cannot get number of frame by using iframe tag. Root cause: {0}", e.getMessage()));
    }
    return ans;
  }
...
//      logger.info(MessageFormat.format("Switched successfully {0} frame located by ''{1}''", 1, locator));
//    } catch (Exception e) {
//      logger.error(
...
//      logger.info(MessageFormat.format("Switched successfully {0} frame located by ''{1}''", 1, id));
//    } catch (Exception e) {
//      logger.error(
//          MessageFormat.format("Cannot switch frame by ''{0}''. Root cause: {1}",
//              id, e.getMessage()));
...
//  }
  ...
  public void dragAndDropByOffset(String locator, int offsetX, int offsetY) {
    WebElement we = findElement(locator);
    logger.info(MessageFormat.format("Dragging web element located by {0} to [{1},{2}]", locator, offsetX, offsetY));
    if (we != null) {
      try {
        Actions actions = new Actions(driver);
        actions.dragAndDropBy(we, offsetX, offsetY).build().perform();
        logger.info(MessageFormat.format("Dropped web element located by {0} at [{1},{2}] successfully", locator, offsetX, offsetY));
      }catch(Exception e){
        logger.error(MessageFormat.format("Can not drag and drop web element located by {0} to [{1},{2}] . Root cause: {3}",
...
//      } catch (Exception e) {
//        logger.error(
//            MessageFormat.format("Cannot click element: {0}. Root cause: {1}", element,
//                e.getMessage()));
//      }
//    }
//  }
  ...
  public void doubleClick(WebElement we) {
    if (we != null) {
      try {
        ...
        logger.error(MessageFormat.format("Cannot perform double-click on element: {0}. Root cause: {1}",
            we, e.getMessage()));
      }
    }
  }
...
//      try {
//        Actions actions = new Actions(driver);
//        actions.dragAndDropBy(sourceElement, xOffset, yOffset).build().perform();
//        logger.info("Dragged and dropped element by offset");
//      } catch (Exception e) {
//        logger.error(MessageFormat.format("Cannot perform drag and drop by offset. Root cause: {0}", e.getMessage()));
//      }
//    }
//  }
...
//  }
  ...
  public void dragAndDropToElement(WebElement sourceElement, WebElement destinationElement) {
    if (sourceElement != null  && destinationElement != null) {
      try {
        ...
        logger.error(MessageFormat.format("Cannot perform drag and drop to target element: {0}. Root cause: {1}", destinationElement, e.getMessage()));
      }
    }
  }
  ...
  public void clickAndHold(WebElement we) {
    if (we != null) {
      try {
        ...
        logger.error(MessageFormat.format("Cannot perform click and hold on element: {0}. Root cause: {1}",
            we, e.getMessage()));
      }
    }
  }
  ...
  public void contextClick(WebElement we) {
    if (we != null) {
      try {
        ...
        logger.error(MessageFormat.format("Cannot perform context click on element: {0}. Root cause: {1}",
            we, e.getMessage()));
      }
    }
  }
  ...
  public void scrollToElementBottom(String locator) {
    ...
    if (we != null) {
      try {
        JavascriptExecutor jsExecutor = (JavascriptExecutor) driver;
        jsExecutor.executeScript("scrollIntoView(false);", we);
        logger.info("Scrolled to element: " + we);
      } catch (Exception e) {
        logger.error(MessageFormat.format("Cannot scroll to element: {0}. Root cause: {1}",
            we, e.getMessage()));
  ...
  public void scrollByAmount(int xOffset, int yOffset) {
    try {
      ...
      logger.error(MessageFormat.format("Cannot scroll by amount. Root cause: {0} ", e.getMessage()));
    }
  }
  ...
  public void scrollFromOrigin(int xOffset, int yOffset) {
    try {
      ...
      logger.error(MessageFormat.format("Cannot scroll from origin. Root cause: {0}", e.getMessage()));
    }
  }
...
//      }
  ...
  public void moveMouseToElement(By locator){
    logger.info(MessageFormat.format("Moving mouse to center of element located: ''{0}''", locator.toString()));
    try{
      Actions actions = new Actions(driver);
      WebElement element = findElement(locator);
      actions.moveToElement(element).perform();
      logger.info(MessageFormat.format("Moved mouse to center of element located: ''{0}'' successfully", locator.toString()));
    }catch (Exception e){
      logger.error(MessageFormat.format("Can not move to element located ''{0}''. Root cause: {1}", locator.toString() ,e.getMessage()));
    }
  }
...
//          Arrays.asList(we));
  ...
  public boolean verifyButtonRedirectToAnotherWebPage(String locator){
    ...
    try {
      ...
      logger.error(MessageFormat.format("Button can not redirect to another web page. Root cause: {0}", e.getMessage()));
    }
    return true;
  }
    ...
    if(we != null){
      try{
        ...
        if(value != null && !value.isEmpty()){
          logger.info(MessageFormat.format("Web element located by ''{0}'' has attribute ''{1}''", locator, attribute));
          AllureLogger.info(MessageFormat.format("Web element located by ''{0}'' has attribute ''{1}''", locator, attribute));
          return true;
        }
        else{
          logger.info(MessageFormat.format("Web element located by ''{0}'' has not attribute ''{1}''", locator, attribute));
          AllureLogger.info(MessageFormat.format("Web element located by ''{0}'' has not attribute ''{1}''", locator, attribute));
        }
      }catch (Exception e){
        logger.error("Cannot determine if element is enabled: " + locator.toString() + ". Root cause: {0}", e.getMessage());
      }
    }
    return false;
  }
...
//    WebElement we = findElement(findBy(locator)); //findBy tra ve kieu By
    int waitTime = timeOut.length > 0 ? timeOut[0] : defaultTimeOut; // ? :
    try {
      logger.info(
          MessageFormat.format("Waiting for web element located by ''{0}'' visible", locator));
